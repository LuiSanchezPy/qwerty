...
require_once 'includes/class-pagopar-gateway-base.php';
require_once 'includes/class-pagopar-tarjetas-guardadas.php';
require_once 'includes/class-pagopar-tarjetas-promocion.php';
require_once 'includes/class-pagopar-tarjetas.php';
require_once 'includes/class-pagopar-upay.php';
require_once 'includes/class-pagopar-bancard-qr.php';
require_once 'includes/class-pagopar-transferencia.php';
require_once 'includes/class-pagopar-efectivo.php';
require_once 'includes/class-pagopar-billeteras.php';
require_once 'includes/class-pagopar-pix.php';
...
class Pagopar_Gateway extends WC_Payment_Gateway
{
    ...
    public function init_form_fields()
    {
        ...
        $pp_admin_fields = $adminHelper->getAdminFields(
            $cities_wc_format,
            $comerciosHijosJson,
            $estadosExistentesPedido,
            $formasPagoJson,
            $formasPagoFechaActualizacion,
            $horarios,
            obtenerURLPaginaConfirmURL(),
            obtenerURLPaginaRedireccionamiento()
        );
...
?><tr valign="top">
            ...
            <td class="forminp">
                <fieldset>
                    ...
                    <button class="<?php echo esc_attr($data['class']); ?>" type="button" name="<?php echo esc_attr($field); ?>" id="<?php echo esc_attr($field); ?>" style="<?php echo esc_attr($data['css']); ?>" <?php echo esc_html($this->get_custom_attribute_html($data)); ?>><?php echo wp_kses_post($data['title']); ?></button>
...
';*/
                ...
                if ($this->id === 'pagopar_transferencia_bancaria') {
                    ...
                    // Condiciones adicionales para $this->id
                    if ($this->id === 'pagopar_upay') {
                        if (isset($this->promociones)) {
                            foreach ($this->promociones as $promocion) {
                                if ($promocion['nombre'] === 'ueno_40') {
                                    $icon_html .= wp_kses_post(str_replace("DIR_URL", plugin_dir_url(__FILE__), $promocion['descripcion_html']));
                                    break;
                                }
                            }
                        }
                    } elseif ($this->id === 'pagopar_bancard_qr' || $this->id === 'pagopar_tarjetas') {
                        $icon_html .= '
                            <span class="promo">Hasta <strong>12 cuotas</strong> sin intereses con tarjetas de crédito Banco Familiar.</span>';
                    }
                    ...
                    foreach ($arrayFormasPagos as $key => $value) {
                        foreach ($value['imagen_principal'] as $key2 => $value2) {
                            $icon_html .= '<span class="method_item"><img src="' . esc_url($value2['url']) . '" alt=""></span>';
                        }
                        break;
                    }
            ...
            public function get_transaction_url_shortcode($order, $soloCalcularFlete = false)
            {
...
calcular flete y se guarda en sesion*/
            ...
            $ciudad_id = 1;
                ...
                $comprador = array(
                    "nombre" => $_POST['billing_first_name'] . ' ' . $_POST['billing_last_name'],
                    "ciudad" => $ciudad_id,
                ...
                if ($_POST['payment_method'] == "pagopar_tarjetas_promocion") {
                    if (isset($_POST["id_promocion"]) && !empty($_POST["id_promocion"])) {
                        ...
                        foreach ($newOrderPagopar['compras_items'] as $valor_item) {
                            ...
                            $total_detalle += ($valor_item['cantidad'] * $valor_item['precio_total']);
                            $precioAnterior += ($valor_item['cantidad'] * $valor_item['precio_total']);
                            $costo_envio += $valor_item['costo_envio'];
                            $opciones_envio = $valor_item['opciones_envio'];
                        }

                        $total_detalle = $total_detalle - (($total_detalle * $porcentaje_decuento) / 100);
                        $totalDescuento = (($precioAnterior * $porcentaje_decuento) / 100);
                        $total_detalle += $costo_envio;  // Sumar costo de envío
                        $newOrderPagopar['compras_items'] = [];

                        if ($cantidad_detalle > 1) {
                            $descripcion_detalle = $descripcion_detalle . " y otros productos más..";
                        }
        ...
        die('aa');*/
                ...
                $newOrderPagopar['token'] = $this->generateOrderHash(
                    $newOrderPagopar['id_pedido_comercio'],
                    $newOrderPagopar['monto_total'],
                    $payments['pagopar']->settings['private_key']
                );
                ...
                $db->insertTransaction(
                    $order_id,
                    $newOrderPagopar['tipo_pedido'],
                    $newOrderPagopar['monto_total'],
                    $hashOrder,
                    $newOrderPagopar['fecha_maxima_pago'],
                    $newOrderPagopar['descripcion_resumen']
                );
             ...
             * @return string
             */
            public function get_transaction_url_blocks($order, $soloCalcularFlete = false)
            {
                $order->update_meta_data('pagopar_tipo_pago', $_POST['pagopar_tipo_pago']);
                #ini_set('display_errors', 'on');
                #error_reporting(E_ALL & ~ E_NOTICE);
                $pagopar_tipo_pago = $order->get_meta('pagopar_tipo_pago');
                $pagopar_metodo_title = $order->data["payment_method"];
...
calcular flete y se guarda en sesion*/
                ...
                $prefijoOrderMetaBlocks = "_wc_other/pagopar/";
                # Si se definio un campo alternativo para documento, usamos ese
                if (($documentoAlternativo != '') and ($documentoAlternativo != 'billing_documento')) {
                    $doc = $order->get_meta($prefijoOrderMetaBlocks . $documentoAlternativo);
                    $nombreCampoDocumento = $prefijoOrderMetaBlocks . $documentoAlternativo;
                } else {
                    $doc = $order->get_meta($prefijoOrderMetaBlocks . 'billing_documento');
                    $nombreCampoDocumento = $prefijoOrderMetaBlocks . 'billing_documento';
                }
                $addr = $order->shipping_address_1;
                if (trim($addr) == '') {
                    $addr = $order->billing_address_1;
                }
                $addRef = $order->get_meta($prefijoOrderMetaBlocks . 'billing_referencia');


                # Si se definio un campo alternativo para ruc, usamos ese
                if (($rucAlternativo != '') and ($rucAlternativo != 'billing_ruc')) {
                    //$buyer->ruc = $_POST[$rucAlternativo];
                    $ruc = $order->get_meta($prefijoOrderMetaBlocks . $rucAlternativo);
                    $nombreCampoRuc = $prefijoOrderMetaBlocks . $rucAlternativo;
                } else {
                    //$buyer->ruc = ($_POST['billing_ruc']) ? $_POST['billing_ruc'] : null;
                    $ruc = ($order->get_meta($prefijoOrderMetaBlocks . 'billing_ruc')) ? $order->get_meta($prefijoOrderMetaBlocks . 'billing_ruc') : null;
                    $nombreCampoRuc = $prefijoOrderMetaBlocks . 'billing_ruc';
                }

                # Si se definio un campo alternativo para razon social, usamos ese
                if (($razonSocialAlternativo != '') and ($razonSocialAlternativo != 'billing_razon_social')) {
                    //$buyer->socialReason = $_POST[$razonSocialAlternativo];
                    $socialReason = $order->get_meta($prefijoOrderMetaBlocks . $razonSocialAlternativo);
                    $nombreCampoRazonSocial = $prefijoOrderMetaBlocks . $razonSocialAlternativo;
                } else {
                    //$buyer->socialReason = ($_POST['billing_razon_social']) ? $_POST['billing_razon_social'] : null;
                    $socialReason = ($order->get_meta($prefijoOrderMetaBlocks . 'billing_razon_social')) ? $order->get_meta($prefijoOrderMetaBlocks . 'billing_razon_social') : null;
                    $nombreCampoRazonSocial = $prefijoOrderMetaBlocks . 'billing_razon_social';
                }
                ...
                if ($woocommerce_ship_to_destination === 'billing_only' || $woocommerce_ship_to_destination === 'billing') {
                    $state_code = $order->data["billing"]["state"];
                } else {
                    $state_code = $order->data["shipping"]["state"];
                }
                ...
                //temp
                // verificar si hace falta enviar ciudad ya que si no es aex va a enviar un id que no existe en pagopar, sino, enviar 1 por defecto
                $coordenadas = $order->get_meta($prefijoOrderMetaBlocks . 'billing_coordenadas');
                $comprador = array(
            ...
            if (is_array($itemPagopar)){
                ...
                $formaPagoFinal = $_POST['pagopar_tipo_pago'];
                ...
                date_default_timezone_set("America/Asuncion");
                $id_tipo = null;
                if ($pagopar_metodo_title == "pagopar_tarjetas_promocion") {
                    if (isset($_POST["id_promocion"]) && !empty($_POST["id_promocion"])) {
                        global $wpdb;
                        $tabla = $wpdb->prefix . 'pagopar_promociones_tarjetas';
                        ...
                        $total_detalle = $total_detalle - (($total_detalle * $porcentaje_decuento) / 100);
                        $totalDescuento = (($precioAnterior * $porcentaje_decuento) / 100);
                        $descripcionProducto = '<p><b>Precio anterior:</b> <del>' . number_format($precioAnterior) . ' Gs.</del></p><p> <b>Precio con descuento:</b> ' . number_format($total_detalle) . ' Gs.</p>';
                        $total_detalle += $costo_envio;  // Sumar costo de envío
                        $newOrderPagopar['compras_items'] = [];
        ...
        $order->update_meta_data('billing_coordenadas', $_POST['billing_coordenadas']);*/
			...
			update_post_meta($order_id, $nombreCampoDocumento, $doc);
                ...
                update_post_meta($order_id, $nombreCampoRazonSocial, $socialReason);
                update_post_meta($order_id, $nombreCampoRuc, $ruc);
                update_post_meta($order_id, 'billing_coordenadas', $order->get_meta($prefijoOrderMetaBlocks . 'billing_coordenadas'));
            ...
            foreach ($ids_recurrentes as $id)
            {
                ...
                $formaPagoFinal = $id_tipo ?? $_POST["pagopar_tipo_pago"];
                ...
                if ($pagopar_metodo_title === "pagopar_tarjetas_guardadas") {
                    #ini_set('display_errors', 'on');
                    #error_reporting(E_ALL);
     ...
     */
        ...
        if($pos !== false) {
            ...
            public function process_payment($order_id)
            {
                ...
                if (is_numeric($idOrdenPedidoYaCreado)) {
                    ...
                    global $wpdb;
                    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Consulta directa necesaria para la tabla personalizada wp_transactions_pagopar del plugin Pagopar, sin equivalente en APIs de WordPress.
                ...
                $checkout_status = $this->es_checkout_blocks();

                if ($checkout_status['uses_blocks']) {
                    return array(
                        'result' => 'success',
                        'redirect' => $this->get_transaction_url_blocks($order)
                    );
                } elseif ($checkout_status['uses_shortcode_in_block'] || $checkout_status['uses_classic_shortcode']) {
                    return array(
                        'result' => 'success',
                        'redirect' => $this->get_transaction_url_shortcode($order)
                    );
                } else {
                    return array(
                        'result' => 'success',
            ...
            public function es_checkout_blocks() {
                // Inicializar el resultado
                $result = [
                    'uses_blocks' => false,
                    'uses_shortcode_in_block' => false,
                    'uses_classic_shortcode' => false,
                ];

                // Verificar si la solicitud usa la Store API (indicador de WooCommerce Blocks)
                $is_store_api_request = wc()->is_store_api_request();

                // Verificar si el formulario de checkout clásico está presente en $_POST
                $is_checkout_form = isset($_POST['woocommerce_checkout_place_order']);

                // Verificar si el sistema está configurado para usar bloques por defecto
                $is_block_default = class_exists('Automattic\WooCommerce\Blocks\Utils\CartCheckoutUtils')
                    && \Automattic\WooCommerce\Blocks\Utils\CartCheckoutUtils::is_checkout_block_default();

                // Verificar si la página usa un constructor de páginas
                $checkout_page_id = wc_get_page_id('checkout');
                $is_page_builder = $this->is_page_builder_active($checkout_page_id);

                // Lógica de detección
                if ($is_store_api_request || ($is_block_default && !$is_checkout_form && !$is_page_builder)) {
                    // Caso 1: WooCommerce Blocks (Store API o configuración de bloques sin formulario de shortcode)
                    $result['uses_blocks'] = true;
                } elseif ($is_checkout_form && ($is_page_builder || $is_block_default)) {
                    // Caso 2: Shortcode en un contexto de constructor de páginas o bloques
                    $result['uses_shortcode_in_block'] = true;
                } elseif ($is_checkout_form) {
                    // Caso 3: Shortcode clásico
                    $result['uses_classic_shortcode'] = true;
                } else {
                    // Caso 4: Por defecto, asumir shortcode clásico
                    $result['uses_classic_shortcode'] = true;
                }
            ...
            /**
             * Verificar si la página usa un constructor de páginas (Elementor, Divi, etc.)
             */
            private function is_page_builder_active($page_id) {
                // Verificar Elementor
                if (class_exists('\Elementor\Plugin') && \Elementor\Plugin::$instance->db->is_built_with_elementor($page_id)) {
                    return true;
                }
                // Verificar Divi
                if (function_exists('et_is_builder_plugin_active') || get_post_meta($page_id, '_et_pb_use_builder', true) === 'on') {
                    return true;
                }
                // Verificar WPBakery
                if (defined('WPB_VC_VERSION') && get_post_meta($page_id, '_wpb_vc_js_status', true) === 'true') {
                    return true;
                }
                // Verificar CartFlows
                if (class_exists('Cartflows_Loader') && get_post_meta($page_id, '_wp_page_template', true) === 'cartflows-default') {
                    return true;
                }
                return false;
            }
          ...
          '."</p><br></div>"; */
            ...
            public function payment_fields()
            {
                ...
                // Booleano para ocultar la seccion de datos adicionales del medio de pago si solo tiene un elemento
                $ocultarDatosAdicionales = !isset($this->datos_adicionales) || count($this->datos_adicionales) <= 1;

                ?><ul class="pagopar_payments" <?php if ($ocultarDatosAdicionales) :  echo ' style="display:none;" ';
                                                endif; ?>>
            <?php foreach ($this->datos_adicionales as $key => $value) : ?>
                ...
                <li>
                    <?php if (!$ocultarDatosAdicionales) :  ?>

                        <label for="sub_payment_method_<?php echo esc_attr($value['forma_pago']); ?>">
                            ...
                            <?php else: ?>
                                <input id="sub_payment_method_<?php echo esc_attr($value['forma_pago']); ?>" type="radio" class="input-radio" name="sub_payment_method_<?php echo esc_attr($this->id); ?>" value="<?php echo esc_attr($value['forma_pago']); ?>" data-order_button_text="">
                                <?php echo esc_html($value['titulo']); ?>
                            <?php endif ?>
                        </label>
                    <?php endif ?>
                    <?php if ($ocultarDatosAdicionales) :  ?>
                    <?php else: ?>
                        ...
                        <span class="methods_group">
                            <?php foreach ($value['imagen'] as $key2 => $value2) : ?>
                                <span class="method_item <?php echo esc_attr($value2['class']); ?>">
                                    <?php if ($value2['url'] != '') : ?>
                                        <img src="<?php echo esc_url($value2['url']); ?>" alt="">
                                    <?php endif ?>
                                </span>
                            <?php endforeach; ?>
                            <?php if ($value2['class'] != ''): ?>
        ...
        <p class="pagopar-copy">Procesado por Pagopar <img src="<?php echo esc_url($urlBasePlugin); ?>images/medios-pagos/iso-pagopar.png" alt="Pagopar"></p>
        ...
        <?php else: ?>
            ...
            <?php endif; ?><?php
                            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- El contenido puede incluir scripts jQuery necesarios para el funcionamiento del plugin y no puede ser escapado sin romper la funcionalidad.
                            echo ob_get_clean();
                        }
                    } ?>