...
function obtenerDatosComerciosWsUpdate($publicKey, $privateKey)
{
    ...
    $usarDatosCacheados = true;

    // Si no existen datos cacheados, o pasaron más de 1440 minutos (24hs)
    if ($datosComercio === false || $fechaDatosComerciosActualizacion === false) {
        $usarDatosCacheados = false;
    } else {
        $minutosDiferencia = (strtotime(date('Y-m-d H:i:s')) - strtotime($fechaDatosComerciosActualizacion)) / 60;
        if ($minutosDiferencia > 180) {
            $usarDatosCacheados = false;
        }
    }

    if (!$usarDatosCacheados) {
        $datosComercioWs = json_decode(traerDatosComercio(), true);
        if ($datosComercioWs['respuesta'] === true) {
            ...
            if ($formasPagoWS['respuesta'] === true) {
                ...
                if ($formasPagoWSArray['respuesta'] === true) {
                    ...
                    update_option('pagopar_datos_comercios_json', json_encode([
                        'contrato_firmado' => $datosComercioWs['resultado']['contrato_firmado']
                    ]));
                    update_option('pagopar_fecha_datos_comercios_actualizacion', date('Y-m-d H:i:s'));
    ...
    // Armamos el resultado con datos cacheados o actualizados
    $datosComercioResultado = [
        'datos_comercios_array' => json_decode($datosComercio, true),
        'formas_pagos_habilitados_array' => json_decode($datosFormasPagosHabilitados, true),
        'fecha_actualizacion' => $fechaDatosComerciosActualizacion,
    ];

    // ✅ Parte obligatoria: eliminar forma_pago 13 y 15 (pago móvil e infonet)
    $bkMediosPagos = $datosComercioResultado['formas_pagos_habilitados_array'];
    $newFormasPago = [];
    foreach ($bkMediosPagos as $value) {
        if ($value['forma_pago'] != 13 && $value['forma_pago'] != 15) {
            $newFormasPago[] = $value;
        }
    }
    ...
    if (!empty($allowed_gateways['pagopar'])) {
        ...
        if (count((array)$formasPagosDisponibles) >= 1) {
            ...
            foreach ($formasPagosDisponibles as $key => $value) {
                ...
                if (isset($value['promociones'])) {
                    $formasPagosDisponiblesAgrupados[$agrupacion][$contador]['promociones'] = $value['promociones'];
                }
            ...
            if (($datosComerciosUpdate['datos_comercios_array']['contrato_firmado'] === true) and (is_user_logged_in())) {
                ...
                unset($allowed_gateways['pagopar_tarjetas_guardadas']);
            }
            ...
            if (is_array($formasPagosDisponiblesAgrupados['upay'])) {
                ...
                if (isset($formasPagosDisponiblesAgrupados['upay'][0]['promociones'])) {
                    $allowed_gateways['pagopar_upay']->promociones = $formasPagosDisponiblesAgrupados['upay'][0]['promociones'];
                }
            } else {
                unset($allowed_gateways['pagopar_upay']);
            }
            ...
            if (is_array($formasPagosDisponiblesAgrupados['tarjetas'])) {
                ...
                unset($allowed_gateways['pagopar_tarjetas']);
            }
            ...
            if (is_array($formasPagosDisponiblesAgrupados['bancard_qr'])) {
                ...
                unset($allowed_gateways['pagopar_bancard_qr']);
            }
            ...
            if (is_array($formasPagosDisponiblesAgrupados['transferencia_bancaria'])) {
                ...
                unset($allowed_gateways['pagopar_transferencia_bancaria']);
            }
            ...
            if (is_array($formasPagosDisponiblesAgrupados['efectivo'])) {
                ...
                unset($allowed_gateways['pagopar_efectivo']);
            }
            ...
            if (is_array($formasPagosDisponiblesAgrupados['billeteras'])) {
                ...
                unset($allowed_gateways['pagopar_billeteras']);
            }
            ...
            if (is_array($formasPagosDisponiblesAgrupados['pix'])) {
                ...
                unset($allowed_gateways['pagopar_pix']);
            }


            #Consultar si exsite promociones registradas
            global $wpdb;
            ...
            if ($results) {
                foreach ($results as $key => $row) {
                    ...
                    $allowed_gateways['pagopar_tarjetas_promocion'] = clone $all_gateways['pagopar'];
                    $allowed_gateways['pagopar_tarjetas_promocion']->method_title = $row['descripcion'];
                    $allowed_gateways['pagopar_tarjetas_promocion']->title = $row['descripcion'];
                    $allowed_gateways['pagopar_tarjetas_promocion']->id = "pagopar_tarjetas_promocion";

                    $formasPagosDisponiblesAgrupadosCabecera['pagopar_tarjetas_promocion'][0]['titulo'] = $row['descripcion'];
                    $formasPagosDisponiblesAgrupadosCabecera['pagopar_tarjetas_promocion'][0]['imagen_principal'] = $imagenPrincipal;
                    $formasPagosDisponiblesAgrupadosCabecera['pagopar_tarjetas_promocion'][0]['descripcion_principal'] = 'Promoción valida desde el ' . date("d/m/Y H:i:s", strtotime($row['inicio_promocion'])) . ' hs hasta el ' . date("d/m/Y H:i:s", strtotime($row['fin_promocion'])) . ' hs';
                    $formasPagosDisponiblesAgrupadosCabecera['pagopar_tarjetas_promocion'][0]['id_promocion'] = $row['id'];
                    $formasPagosDisponiblesAgrupadosCabecera['pagopar_tarjetas_promocion'][0]['porcentaje'] = $row['porcentaje'];

                    $allowed_gateways['pagopar_tarjetas_promocion']->datos_adicionales = $formasPagosDisponiblesAgrupadosCabecera['pagopar_tarjetas_promocion'];
                    $allowed_gateways['pagopar_tarjetas_promocion']->datos_adicionales_agrupados = $formasPagosDisponiblesAgrupadosCabecera['pagopar_tarjetas_promocion'];
                }
            } else {
                unset($allowed_gateways['pagopar_tarjetas_promocion']);
            }
...
/**
 * Mostrar promoción en la sección de visualización de producto para forma pago 26 con promoción de nombre ueno_40
 *
 * @author Carlos Perez
 * @version 1 2025-01-25
 * @return void
 */
function mostrar_promo_producto()
{
    $formasPagosDisponibles = json_decode(get_option('pagopar_formas_pagos_habilitados_json'), true);
    if (!is_product() || empty($formasPagosDisponibles)) {
        return;
    }
    foreach ($formasPagosDisponibles as $value) {
        if ($value['forma_pago'] == 26 && isset($value['promociones'])) {
            foreach ($value['promociones'] as $promocion) {
                if ($promocion['nombre'] === 'ueno_40') {
                    echo wp_kses_post($promocion['descripcion_html']);
                    break;
                }
            }
            break;
...
add_action('woocommerce_before_add_to_cart_form', 'mostrar_promo_producto');