...
if (settingsPagopar.medios_pago.hasOwnProperty(medio_pago)) {
    ...
    const Label = () => {
        ...
        // Obtiene la descripción y las imágenes directamente de datos_adicionales
        const firstAdditional = value.datos_adicionales[Object.keys(value.datos_adicionales)[0]];
            images = firstAdditional.imagen.map(img => (
                createElement('span', { className: 'method_item' },
                    createElement('img', { src: img.url, alt: '', className: img.class || '' })
                )
            ));
    ...
    const Content = (props) => {
        ...
        const additionalContent = additionalKeys.map((adKey) => {
            ...
            return createElement('li', { key: adKey, style: { display: isSingleItem ? 'none' : '' } },
                createElement('label', {
