...
if (settingsPagopar.medios_pago.hasOwnProperty(medio_pago)) {
    ...
    const Label = () => {
        ...
        const description = value.datos_adicionales_agrupados[0].descripcion_principal;
        ...
        // Crer el elemento solo si tiene un array promociones y existe uno con el nombre ueno_40
        if (value.datos_adicionales[0].promociones && value.datos_adicionales[0].promociones.length > 0) {
            for (const promocion of value.datos_adicionales[0].promociones) {
                if (promocion.nombre === "ueno_40") {
                    promo = createElement('div', {
                        dangerouslySetInnerHTML: { __html: promocion.descripcion_html }
                    });
                    break;
                }
            }
            ...
            const unsubscribe = onPaymentSetup(async () => {
                ...
                if (selectedPaymentMethod) {
                    ...
                    return {
                        ...
                        meta: {
                            paymentMethodData: {
                                "wc-pagopar_upay-new-payment-method": selectedValue // Enviar el valor del input seleccionado
                            },
                        },
                    };
                }
