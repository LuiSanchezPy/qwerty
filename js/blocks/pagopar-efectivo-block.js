medio_pago = 'pagopar_efectivo';

if (settingsPagopar.medios_pago.hasOwnProperty(medio_pago)) {
    const value = settingsPagopar.medios_pago[medio_pago];

    const Label = () => {
        const firstKey = Object.keys(value.datos_adicionales_agrupados)[0];
        const firstAdditional = value.datos_adicionales_agrupados[firstKey];
        const description = firstAdditional.descripcion_principal;
        const images = firstAdditional.imagen_principal ? firstAdditional.imagen_principal.map(img => (
    ...
    const Content = (props) => {
        ...
        useEffect(() => {
            ...
            const unsubscribe = onPaymentSetup(async () => {
                ...
                props.activePaymentMethod = "pagopar";
        ...
        const additionalContent = Object.keys(additionalData).map(key => {
