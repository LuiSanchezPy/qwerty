...
function volverEnviarProductosDireccionGlobal() {
    ...
    $args = array(
        ...
        'meta_query' => array(
            array(
                'key' => 'direccion_global_utilizada',
                'value' => '1',
                'compare' => '='
            )
        ),
    ...
    $productosDireccionGlobal = array_map(function($id) {
...
function volverEnviarProductosConEnvioFallido() {
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Consulta directa necesaria para la tabla personalizada wp_pagopar_sincronizacion_log_enviado del plugin Pagopar, sin equivalente en APIs de WordPress.
...
function importarProducto($json_pagopar) {
    ...
    if (is_numeric($existeProducto[0]->ID)) {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Inserción directa en wp_posts necesaria para evitar bucles infinitos en el hook save_post durante la sincronización de Pagopar.
        $a = $wpdb->query($wpdb->prepare("insert into " . $wpdb->prefix . "posts (post_title, post_content, post_status, post_type, post_date, post_date_gmt) values (%s, %s, 'publish', 'product', now(), now())", $json_pagopar['datos']['titulo'], $json_pagopar['datos']['descripcion']));
        $productoID = $wpdb->insert_id;

        # Definimos slug
        $urlAmigable = sanitize_title($json_pagopar['datos']['titulo'], $productoID);
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Actualización directa en wp_posts necesaria para evitar bucles infinitos en el hook save_post durante la sincronización de Pagopar.
        $a = $wpdb->query($wpdb->prepare("update " . $wpdb->prefix . "posts set post_name = %s where ID = %s", $urlAmigable, $productoID));
        $tipoAccion = 'creacion';
    }
	...
	$conversion_dolar_guarani = get_option('pagopar_conversion_dolar_guarani');
	$monto = $json_pagopar['datos']['monto'];
	$monto_lista = $json_pagopar['datos']['monto_lista'];

	# Si Woocommerece esta en dolares y tasa de conversion esta configurada, hacer conversion de guarani a dolares
	if (get_woocommerce_currency() === 'USD' && !is_null($conversion_dolar_guarani) && $conversion_dolar_guarani !== "" && $conversion_dolar_guarani > 0) {
		$monto = $monto / $conversion_dolar_guarani;
		if ($monto_lista > 0) {
			$monto_lista = $monto_lista / $conversion_dolar_guarani;
		}
	}

  if ($json_pagopar['datos']['monto_lista']>0){
        update_post_meta($productoID, '_price', $monto);
        update_post_meta($productoID, '_regular_price', $monto_lista);
        update_post_meta($productoID, '_sale_price', $monto);

    }else{
        update_post_meta($productoID, '_price', $monto);
        update_post_meta($productoID, '_regular_price', $monto);
        update_post_meta($productoID, '_sale_price', '');
    }    
    ...
    if (($json_pagopar['datos']['activo'] === '0') or ( $json_pagopar['datos']['activo'] === false) or ( $json_pagopar['datos']['activo'] === 'false')) {
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Actualización directa en wp_posts necesaria para evitar bucles infinitos en el hook save_post durante la sincronización de Pagopar.
        $wpdb->query($wpdb->prepare("update " . $wpdb->prefix . "posts set post_status = %s where ID = %s", 'draft', $productoID));    
    }
...
function exportarProductoInicial($post_id, $post, $update, $enviarLogsPagopar = true, $soloRetornarError = false) {
  ...
  # si tiene descuento, enviamos ese monto como precio/monto de venta
    ...
    if(get_woocommerce_currency() !=='PYG' && get_woocommerce_currency() !== 'USD'){
        $resultadoError['respuesta'] = false;
        $resultadoError['resultado'] = 'Woocommerece no está configurado en moneda Guaranies o Dólares';
        return $resultadoError;
    }
	...
	# Si Woocommerece esta configurado en dolares
	if (get_woocommerce_currency() === 'USD') {
		$conversion_dolar_guarani = get_option('pagopar_conversion_dolar_guarani');

		if (is_null($conversion_dolar_guarani) || $conversion_dolar_guarani === '' || $conversion_dolar_guarani <= 0) {
			$resultadoError['respuesta'] = false;
			$resultadoError['resultado'] = 'La conversión de dólar a guaraní no está configurada';

			return $resultadoError;
		}

		# Convertir el precio del dolar a guarani
		$precio = $precio * $conversion_dolar_guarani;
		if ($precioDescuento !== null) {
			$precioDescuento = $precioDescuento * $conversion_dolar_guarani;
		}
	}
    ...
    if ($crearActualizarLinkPago['respuesta'] === true) {
	    ...
	    if (get_woocommerce_currency() === 'USD') {
		    update_post_meta($post_id, 'pagopar_monto_conversion_dolar_guarani', get_option('pagopar_conversion_dolar_guarani'));
	    }
...
function agregarLog($array, $accion, $idProducto) {
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Inserción directa necesaria para la tabla personalizada wp_pagopar_sincronizacion_log_enviado del plugin Pagopar, sin equivalente en APIs de WordPress.
    $wpdb->query($wpdb->prepare("insert into " . $wpdb->prefix . "pagopar_sincronizacion_log_enviado "
...
function procesarLogByLogID($logID, $idProducto) {
    ...
    if (trim($existeLogPendienteProducto[0]->log_id) !== '') {
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Actualización directa necesaria para la tabla personalizada wp_pagopar_sincronizacion_log_enviado del plugin Pagopar, sin equivalente en APIs de WordPress.
        $wpdb->query($wpdb->prepare("UPDATE " . $wpdb->prefix . "pagopar_sincronizacion_log_enviado SET log_enviado = 3 WHERE log_id IN (%s)", $existeLogPendienteProducto[0]->log_id));    
    }
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Actualización directa necesaria para la tabla personalizada wp_pagopar_sincronizacion_log_enviado del plugin Pagopar, sin equivalente en APIs de WordPress.
    $wpdb->query($wpdb->prepare("update " . $wpdb->prefix . "pagopar_sincronizacion_log_enviado "
...
function ponerVolverEnviarDireccionModificada($idDireccion) {
    ...
    $args = array(
        ...
        'meta_query' => array(
            array(
                'key' => 'pagopar_direccion_id_woocommerce',
                'value' => $idDireccion,
                'compare' => '='
            )
        ),
    ...
    $productosAfectados = array_map(function($id) {
...
function volverEnviarProductosDatosDireccionModificados() {
    ...
    $args = array(
        ...
        'meta_query' => array(
            array(
                'key' => 'pagopar_volver_enviar',
                'value' => 'direccion_modificada',
                'compare' => '='
            )
        ),
        'orderby' => 'ID',
    ...
    $productosDireccionGlobal = array_map(function($id) {
        return (object) array(
            'post_id' => $id,
            'meta_value' => 'direccion_modificada'
        );
    }, get_posts($args));  
...
function enviarProductosSinStock() {
    ...
    $args = array(
        ...
        'meta_query' => array(
            ...
            array(
                'key' => 'pagopar_producto_sin_stock_importado',
                'value' => '1',
                'compare' => '!=',
                'type' => 'NOT EXISTS'
            )
...
function enviarProductosDescuento() {
    ...
    $args = array(
        ...
        'meta_query' => array(
            ...
            array(
                'key' => 'pagopar_producto_descuento_importado',
                'value' => '1',
                'compare' => '!=',
                'type' => 'NOT EXISTS'
            )
    ...
    $productosDireccionGlobal = array_map(function($id) {
        return (object) array(
            'post_id' => $id,
            'meta_value' => get_post_meta($id, '_sale_price', true)
        );
    }, get_posts($args));
...
function volverEnviarProductosDatosDireccionGlobalModificados() {
    ...
    $args = array(
        ...
        'meta_query' => array(
            array(
                'key' => 'pagopar_volver_enviar',
                'value' => 'direccion_global_modificada',
                'compare' => '='
            )
        ),
        'orderby' => 'ID',
    ...
    $productosDireccionGlobal = array_map(function($id) {
        return (object) array(
            'post_id' => $id,
            'meta_value' => 'direccion_global_modificada'
        );
    }, get_posts($args));   
...
/**
 * Establecer la bandera 'pagopar_cambio_divisa' a 1 en postmeta cuando se hace modificacion en la divisa
 *
 * @author Carlos Perez
 * @since 2025-02-17
 * @return void
 */
function establecerVolverAEnviarPorCambioDivisa() {
	global $wpdb;
    $args = array(
        'post_type' => 'product',
        'posts_per_page' => -1,
	...
	foreach ($productosAfectados as $key => $value) {
		update_post_meta($value->ID, 'pagopar_cambio_divisa', '1');
	}
}

/**
 * Obtener y exportar los productos marcados para volver a enviar en el momento de la modificacion en el cambio de divisa
 *
 * @author Carlos Perez
 * @since 2025-02-17
 * @return array
 */
function volverEnviarProductosPorCambioDivisa() {
	global $wpdb;
    ...
    }, get_posts($args));	$idProductos = [];

	foreach ($productosDireccionGlobal as $key => $value) {
		$idProductos[$value->post_id] = exportarProductoInicial($value->post_id, get_post($value->post_id), $update, true);
		update_post_meta($value->post_id, 'pagopar_cambio_divisa', '');
	}
