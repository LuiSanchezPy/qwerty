...
$GLOBALS['version'] = 'WOOCOMMERCE 2.8.13';
...
$wpdb->posts WHERE post_type = 'page' AND post_name IN (";
    ...
    $tohide = get_posts($args);
    return $tohide;
}

//INICIO DE CODIGO PARA AGREGAR LOGICA DE BLOQUES PAGOPAR WOOCOMMERCE



function pagopar_gateway_block_support()
{
    ...
    require_once __DIR__ . '/includes/class-wc-pagopar-gateway-blocks-support.php';

    // registering the PHP class we have just included
    add_action(
        'woocommerce_blocks_payment_method_type_registration',
        function (Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry) {
            $payment_method_registry->register(new WC_Pagopar_Gateway_Blocks_Support);
        }
    );
}
...
function pagopar_cart_checkout_blocks_compatibility()
{
...
//FIN DE CODIGO PARA AGREGAR LOGICA DE BLOQUES PAGOPAR WOOCOMMERCE
/**
...
function pagopar_borrar_tarjeta()
{
    ...
    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Outputting raw JSON from trusted API
    echo $response;
}
...
function pagopar_wc_refund_button()
{
    ...
    if (isset($_GET['page']) && $_GET['page'] === 'wc-settings' && isset($_GET['tab']) && $_GET['tab'] === 'checkout') {
        echo '<style>
            tr[data-gateway_id="pagopar_tarjetas"], 
            tr[data-gateway_id="pagopar_upay"],
            tr[data-gateway_id="pagopar_bancard_qr"],
            tr[data-gateway_id="pagopar_transferencia_bancaria"],
            tr[data-gateway_id="pagopar_efectivo"],
            tr[data-gateway_id="pagopar_billeteras"],
            tr[data-gateway_id="pagopar_pix"] {
    ...
    try {
        ...
        if (
            in_array($resultadoObject["forma_pago_identificador"], $array_pagos) &&
...
?>
            <script>
                jQuery(function() {
                    var r = jQuery('<button type="button" id="reversePagoparPay" class="button" name="<?php echo esc_attr($resultadoObject['hash_pedido']) ?>" value="<?php echo esc_attr($orderId) ?>">Reversar pago Pagopar</button>');
...
function pagopar_noticias_admin_notice__error()
{
    ...
    if ($pp_comercioObject->respuesta) {
                ...
                <div class="notice notice-warning is-dismissible">
                    <p>
                        ...
                        echo wp_kses_post(__(
                            '<strong>Pagopar</strong> - Tiene una deuda pendiente de Gs. ' . $pp_comercioObject->resultado->pedidos_pendientes[$x]->monto
                        )); 
                        ?> 
                        <a href="<?php echo esc_url($pp_comercioObject->resultado->pedidos_pendientes[$x]->url); ?>">Ver pedido</a>
                    </p>
                </div>
            <?php
            ...
            <div class="notice notice-warning is-dismissible">
                <p>
                    <?php echo wp_kses_post(__(
                        '<strong>Pagopar</strong> - Tu entorno de desarrollo es Staging. Al estar terminado tu sitio debes pasar a producción, puedes hacerlo tú mismo siguiendo las <a href="https://soporte.pagopar.com/portal/es/kb/articles/pase-a-producci%C3%B3n-en-woocommerce" target="_blank">instrucciones</a> o <a href="https://soporte.pagopar.com/portal/es/newticket?departmentId=387583000000006907">solicitar a Soporte</a> que te ayudemos con dicha tarea.'
                    )); ?>
                </p>
            </div>
        <?php
        ...
        <div class="notice notice-warning is-dismissible">
            <p>
                <?php echo wp_kses_post(__(
                    '<strong>Pagopar</strong> - Si quiere habilitar los couriers ofrecidos por Pagopar o ya estaba utilizando AEX, debe configurar de nuevo debido a la nueva versión del plugin.'
                )); ?>
                <a target="_blank" href="https://soporte.pagopar.com/portal/es/kb/articles/habilitar-couriers-ofrecidos-por-pagopar">Ver pasos de la configuración</a>
            </p>        
        </div>
    <?php
...
function pagopar_other_plugins()
{
    ...
    ?>
        <div class="notice notice-warning is-dismissible">
            <p>
                <?php echo wp_kses_post(__(
                    '<strong>Pagopar</strong> - Al parecer tiene instalado plugins que podrían ocasionar conflictos con el de Pagopar. Te recomendamos leer las <a target="_blank" href="https://soporte.pagopar.com/portal/es/kb/articles/conflicto-con-plugins-de-edici%C3%B3n-de-campos">instrucciones</a> para solucionar dichos conflictos. Si ya realizó esta operación o el equipo de Soporte de Pagopar lo hizo, no tenga en cuenta esta advertencia.'
                )); ?>
            </p>        
        </div>
    <?php
...
function child_plugin_activate()
{
    ...
    if (!is_plugin_active('woocommerce/woocommerce.php') and current_user_can('activate_plugins')) {
        ...
        wp_die('Lo sentimos, pero este plugin requiere <a href="https://wordpress.org/plugins/woocommerce/" target="_blank">WooCommerce</a> para ser activado. <br><a href="' . esc_url(admin_url('plugins.php')) . '">&laquo; Volver a Plugins</a>');
    } else {
...
add_action('before_woocommerce_init', 'pagopar_cart_checkout_blocks_compatibility');
add_filter('', 'disable_shipping_fields_in_blocks', 10, 1);

function disable_shipping_fields_in_blocks($data)
{
    unset($data['shipping']['city']);
    return $data;
}
//ACTION PARA CREAR NUEVOS INPUTS DE PAGOPAR EN EL CHECKOT
add_action('woocommerce_init', function () {

    // Obtener la configuración de Pagopar
    ...
    // Obtener los campos alternativos
    ...
    // Registrar el campo Documento (CI)
    if (empty($documentoAlternativo) || $documentoAlternativo == 'billing_documento') {
        woocommerce_register_additional_checkout_field(
            array(
                'id'            => 'pagopar/billing_documento',
                'label'         => 'Documento (CI)',
                'optionalLabel' => 'Documento (CI) (optional)',
                'location'      => 'order',
                'required'      => true,
                'attributes'    => array(
                    'autocomplete' => 'document-id',
                    'aria-label'   => 'Documento (CI)',
    ...
    // Registrar el campo Razón Social
    if (empty($razonSocialAlternativo) || $razonSocialAlternativo == 'billing_razon_social') {
        woocommerce_register_additional_checkout_field(
            array(
                'id'            => 'pagopar/billing_razon_social',
                'label'         => 'Razón Social',
                'optionalLabel' => 'Razón Social (optional)',
                'location'      => 'order',
                'required'      => false,
                'attributes'    => array(
    ...
    // Registrar el campo RUC
    if (empty($rucAlternativo) || $rucAlternativo == 'billing_ruc') {
        woocommerce_register_additional_checkout_field(
            array(
                'id'            => 'pagopar/billing_ruc',
                'label'         => 'RUC',
                'optionalLabel' => 'RUC (optional)',
                'location'      => 'order',
                'required'      => false,
                'attributes'    => array(
                    'autocomplete' => 'tax-id',
    ...
    // Validar si Mobi está activo y agregar el campo de coordenadas
    $mobi_activo = get_option('pagopar_mobi_activo_general');
    if ($mobi_activo == 1) {
        woocommerce_register_additional_checkout_field(
            array(
                'id'            => 'pagopar/billing_coordenadas',
                'label'         => 'Coordenadas',
                'optionalLabel' => 'Coordenadas (optional)',
                'location'      => 'order',
                'required'      => true,
                'attributes'    => array(
...
//ACTIONS PARA GUARDAR LOS DATOS DENTRO DEL USER_META

add_action(
    'woocommerce_set_additional_field_value',
    function ($key, $value, $group, $wc_object) {
        $user_id = $wc_object->get_user_id(); // Obtener el ID del usuario

        // Guardar el Documento (CI)
        if ('pagopar/billing_documento' === $key) {
            update_user_meta($user_id, 'pagopar_documento', sanitize_text_field($value));
        }

        // Guardar la Razón Social
        if ('pagopar/billing_razon_social' === $key) {
            update_user_meta($user_id, 'pagopar_razon_social', sanitize_text_field($value));
        }

        // Guardar el RUC
        if ('pagopar/billing_ruc' === $key) {
            update_user_meta($user_id, 'pagopar_ruc', sanitize_text_field($value));
        }

        // Guardar las Coordenadas (si se utiliza)
        if ('pagopar/billing_coordenadas' === $key) {
            update_user_meta($user_id, 'pagopar_coordenadas', sanitize_text_field($value));
        }
    },
    10,
    4
...
//ACTIONS PARA OBTENER Y SETEAR LOS VALORES PREDETERMINADOS POR CADA USUARIO
// Filtro para obtener el valor predeterminado del campo Documento (CI)
add_filter(
    "woocommerce_get_default_value_for_pagopar/billing_documento",
    function ($value, $group, $wc_object) {
        $user = wp_get_current_user();
        return get_user_meta($user->ID, 'pagopar_documento', true);
    },
    10,
    3
);

// Filtro para obtener el valor predeterminado del campo Razón Social
add_filter(
    "woocommerce_get_default_value_for_pagopar/billing_razon_social",
    function ($value, $group, $wc_object) {
        $user = wp_get_current_user();
        return get_user_meta($user->ID, 'pagopar_razon_social', true);
    },
    10,
    3
);

// Filtro para obtener el valor predeterminado del campo RUC
add_filter(
    "woocommerce_get_default_value_for_pagopar/billing_ruc",
    function ($value, $group, $wc_object) {
        $user = wp_get_current_user();
        return get_user_meta($user->ID, 'pagopar_ruc', true);
    },
    10,
    3
);

// Filtro para obtener el valor predeterminado del campo Coordenadas (si Mobi está activo)
add_filter(
    "woocommerce_get_default_value_for_pagopar/billing_coordenadas",
    function ($value, $group, $wc_object) {
        $user = wp_get_current_user();
        return get_user_meta($user->ID, 'pagopar_coordenadas', true);
    },
    10,
    3
);
...
add_action('woocommerce_blocks_loaded', 'pagopar_gateway_block_support');
...
function pagopar_init()
{
    ...
    {
        $methods[] = 'Pagopar_Tarjetas_Promocion';
        $methods[] = 'Pagopar_Tarjetas_Guardadas';
        $methods[] = 'Pagopar_Upay';
        $methods[] = 'Pagopar_Tarjetas';
        $methods[] = 'Pagopar_Bancard_QR';
        $methods[] = 'Pagopar_Transferencia';
        $methods[] = 'Pagopar_Efectivo';
        $methods[] = 'Pagopar_Billeteras';
        $methods[] = 'Pagopar_Pix';
    ...
    wp_enqueue_style('url-checkout-blocks-css', plugins_url('css/checkout-blocks.css', __FILE__));
...
function oaf_create_admin_menu()
{
    ...
    add_menu_page(
        ...
        'Pagopar',
        'manage_options',
        'pagopar_create_admin_menu_plugin',
        'pagopar_create_admin_menu_function',
            ...
            <?php if (is_array($comerciosHijosJsonGuardado['resultado'])): ?>
                <select>
                    <?php foreach ($comerciosHijosJsonGuardado['resultado'] as $key => $value): ?>
                        <option value="<?php echo esc_attr($value['token_publico']); ?>"><?php echo esc_html($value['descripcion']); ?></option>
                    <?php
                    endforeach; ?>
                </select>
    ...
    <?php
        ...
        <div class="options_group">
            ...
            <p class="form-field" id="selects_categories_pagopar">
                <label for="product_field_type0"><?php esc_html_e('Seleccionar categoría', 'pagopar'); ?></label>
                <?php
                $final_medidas = 0;
                ...
                ?>
                        <select id="product_field_type<?php echo esc_attr($key); ?>" name="product_field_type<?php echo esc_attr($key); ?>" class="ajax_chosen_select_products" style="width:30%"
                            multiple="multiple" level="<?php echo esc_attr($key + 1); ?>" onchange="verificarSelect('product_field_type<?php echo esc_attr($key); ?>');">
                            <?php
                            foreach ($padres as $padre) {
                                ...
                                echo '<option ' . esc_attr($sel) . ' value="' . esc_attr($padre->categoria) . '" productoFisico="' . intval($padre->producto_fisico) . '" medidas="' . esc_attr($padre->medidas) . '" hijos="' . esc_attr($children). '">' . esc_attr($padre->descripcion) . '</option>';
                            }
                ...
                ?>
                    <select id="product_field_type0" name="product_field_type0" class="ajax_chosen_select_products" style="width:30%"
                        ...
                        foreach ($cats as $padre) {
                            $children = 1;
                            if (empty($padre->hijas)) $children = 0;
                            echo '<option value="' . esc_attr($padre->categoria) . '" medidas="' . esc_attr($padre->medidas) . '" hijos="' . esc_attr($children) . '">' . esc_attr($padre->descripcion) . '</option>';
                        }
                        ?>
                    </select>
                <?php
                ...
                <style type="text/css">
                    ...
                    .form-field.form-row.variable_weight0_field {
                        display: none !important;
                    }

                    .form-field.form-row.variable_weight1_field {
                        display: none !important;
                    }

                    .form-field.form-row.variable_weight2_field {
                        display: none !important;
                    }

                    .form-field.form-row.variable_weight3_field {
                        display: none !important;
                    }

                    .form-field.form-row.variable_weight4_field {
                        display: none !important;
                    }

                    .form-field.form-row.variable_weight5_field {
                        display: none !important;
                    }

                    .form-field.form-row.variable_weight6_field {
                        display: none !important;
                    }

                    .form-field.form-row.dimensions_field {
                        display: none !important;
                    }
                </style>
 ...
 */
                    ...
                    #pagopar_product_data .form-field.dimensions_field {
                        display: none;
                    }

                    #pagopar_product_data .form-field.product_weight_field {
                        display: none;
                    }
                </style>
            <?php
...
*/
                    ...
                    #pagopar_product_data .form-field.product_weight_field+.form-field.dimensions_field {
                        display: block;
                    }
                </style>
            <?php
            ...
            <p class="form-field dimensions_field">
                ...
                <span class="wrap">
                    <input id="pagopar_largo" placeholder="Longitud" class="input-text wc_input_decimal" size="6" type="text"
                        name="pagopar_largo" value="<?php echo esc_attr(get_post_meta($product_id, 'pagopar_largo', true)); ?>">
                    <input placeholder="Anchura" class="input-text wc_input_decimal" size="6" type="text"
                        name="pagopar_ancho" value="<?php echo esc_attr(get_post_meta($product_id, 'pagopar_ancho', true)); ?>">
                    <input placeholder="Altura" class="input-text wc_input_decimal last" size="6" type="text"
                        name="pagopar_alto" value="<?php echo esc_attr(get_post_meta($product_id, 'pagopar_alto', true)); ?>">
                </span>
            </p>
        </div>
    <?php
            ...
            <div class="row">
                <div class="col-lg-2">
                    <p class="form-field">
                        ...
                        <span id="desdePropio"><?php echo esc_html($cities_wc_format[$city_selected]); ?></span>
                    </p>
                </div>
                <div class="col-lg-3">
                    <?php
                ...
                <div id="envios_propios_seleccionados" class="col-lg-12">
                    ...
                    if (!empty($envio_propio)) {
                        foreach ($propios as $propio) {
                            ...
                            $t = $propio[2]; //tiempo
                            $identi = $d . '_' . $c . '_' . $t;
                            echo "<p class='envio_propio' id='" . esc_attr($identi) . "' o='" . esc_attr($city_selected) . "' d='" . esc_attr($d_id) . "' c='" . esc_attr($c) . "' t='" . esc_attr($t) . "'>";
                            echo "Enviar tu producto de <span class='origin_envio_propio'>" . esc_html($cities_wc_format[$city_selected]) . "</span> ";
                            echo "a " . esc_attr($d) . " le costará al cliente " . esc_attr($c) . " Gs. adicionales en " . esc_attr($t) . " Hs. ";
...
function pagopar_categories()
{
    ...
    foreach ($hijas->hijas as $hijo) {
	    ...
	    $output .= '<option value="' . esc_attr($hijo->categoria) . '" productofisico="' . intval($hijo->producto_fisico) . '" medidas="' . esc_attr($hijo->medidas) . '" hijos="' . esc_attr($children) . '">' . esc_html($hijo->descripcion) . '</option>';
    }
	echo $output;
}
...
.footerPagopar {
  ...
  right: 0;
  bottom: 0;
  background-color: ' . esc_attr($colorFondoDefectoTema) . ';
  text-align: center;
  color: #FFF;border-top: 1px solid ' . esc_attr($colorBordeDefectoTema) . ';
}
...
.light .footerPagopar { background-color: ' . esc_attr($colorFondoDefectoTema) . '; border-top: 1px solid ' . esc_attr($colorBordeDefectoTema) . '; }
...
<div class="' . esc_attr($tema) . '">
<div class="footerPagopar">
		<div class="container">
			<div class="pagopar-methods">
				<ul class="list">
                                        
                                        <li class="method"><img title="Pagá con VISA a través de Pagopar" alt="Cobrar con Tarjeta de Crédito Visa en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/visa.png" ></li>

                                        <li class="method"><img title="Pagá con Mastercard a través de Pagopar" alt="Cobrar con Tarjeta de Crédito Mastercard en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/mastercard.png" ></li>

                                        <li class="method"><img title="Pagá con American Express a través de Pagopar" alt="Cobrar con Tarjeta de Crédito American Express en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/aex.png" ></li>

                                        <li class="method extra-method hidden"><img title="Pagá con Diners a través de Pagopar" alt="Cobrar con Tarjeta de Crédito Diners en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/diners.png" ></li>

                                        <li class="method extra-method hidden"><img title="Pagá con Credifielco a través de Pagopar" alt="Cobrar con Tarjeta de Crédito Credifielco en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/credifielco.png" ></li>

                                        <li class="method extra-method hidden"><img title="Pagá con Única a través de Pagopar" alt="Cobrar con Tarjeta de Crédito Única en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/unica.png" ></li>

                                        <li class="method extra-method hidden"><img title="Pagá con Credicard a través de Pagopar" alt="Cobrar con Tarjeta de Crédito Credicard en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/credicard.png" ></li>

                                        <li class="method extra-method hidden"><img title="Pagá con Cabal a través de Pagopar" alt="Cobrar con Tarjeta de Crédito Cabal en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/cabal.png" ></li>

                                        <li class="method extra-method hidden"><img title="Pagá con Panal a través de Pagopar" alt="Cobrar con Tarjeta de Crédito Panal en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/panal.png" ></li>

                                        <li class="method extra-method hidden"><img title="Pagá con Pagopar Card a través de Pagopar" alt="Cobrar con Tarjeta de Crédito Pagopar Card en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/pagopar.png" ></li>

                                        <li class="method"><img title="Pagá con Wepa a través de Pagopar" alt="Cobrar con Wepa en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/wepa.png" ></li>
                                        
                                        <li class="method"><img title="Pagá con Pagoexpress a través de Pagopar" alt="Cobrar con Pagoepxress en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/pagoexpress.png" ></li>

                                        <li class="method"><img title="Pagá con Aquí Pago a través de Pagopar" alt="Cobrar con Aqui Pago en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/aquipago.png" ></li>

                                        <li class="method"><img title="Pagá con Tigo Money a través de Pagopar" alt="Cobrar con Tigo Money en Paraguay - Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/tigo-money.png" ></li>
					
                                        <li class="method"><img title="Pagá con Billetera Personal a través de Pagopar" alt="Cobrar con Billetera Personal en Paraguay - Pagopar"  src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/billetera-personal.png" ></li>					

                                        <li class="method"><img title="Pagá con Giros Claro a través de Pagopar" alt="Cobrar con Giros Claro en Paraguay - Pagopar"  src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/giros-claro.png" ></li>					

					                    <li class="method"><img title="Pagá con Wally a través de Pagopar" alt="Cobrar con Billetera Personal en Paraguay - Pagopar"  src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/wally.png" ></li>					
					...
					<li class="logo-pagopar">
                                        <a target="_blank" href="https://www.pagopar.com/">
                                            <img title="Vender y cobrar online fácil con Pagopar"  "Vender y cobrar online fácil con Pagopar" src="' . esc_url($urlBasePlugin) . 'images/footer/' . esc_attr($tema) . '/procesado-por-pagopar.png" alt="Procesado por Pagopar">
                                        </a>    
                                        </li>
				</ul>
...
function pagopar_add_fees()
{
        ...
        <?php if ($value->amount > 0): ?>
            ...
            <tr class="fee">
                ...
                <td data-title="fee"><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">₲</span>
                        <?php echo esc_html(str_replace(",", ".", number_format($value->amount))); ?></span>
                </td>
            </tr>
        <?php endif; ?>
    <?php endforeach; ?>
    ...
    <?php if ($checkoutFeesForWoocommerce == 1): ?>
        ...
        <?php if ((WC()->cart->fee_total * -1) > 0): ?>
            ...
            <tr class="fee">
                <th><?php echo esc_html($fee_text); ?></th>
                <td data-title="fee"><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">₲</span>
                        <?php echo esc_html(str_replace(",", ".", number_format(WC()->cart->fee_total))); ?></span>
                </td>
            </tr>
    ...
    <?php if ($_POST['porcentaje'] > 0): ?>
        ...
        ?>
        <tr class="fee">
            <th>Descuento <?php echo esc_html($_POST['porcentaje']); ?>% </th>
            <td data-title="fee"><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">₲</span>
                    <?php echo esc_html(str_replace(",", ".", number_format($monto_porcentaje * -1))); ?></span>
            </td>
        </tr>
    ...
    <tr class="order-total">
        <th>Total</th>
        <td data-title="Fee"><span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">₲</span>
                <strong><?php echo esc_html(str_replace(",", ".", number_format($monto_foot))) . esc_html($labelTaxesTotal); ?></strong></span>
        </td>
    </tr>
...
function show_flete_in_checkout()
{
    ...
    foreach (
        ...
        if ($pagopar_parent_id == 906) { //Si la categoria padre es igual a producto, mostrar el flete.
            return true;
        }
    }
    return false;
...
function ajax_pagopar_enqueue_scripts()
{
    ...
    // Aquí obtenemos los valores de configuración o parámetros de PHP
    $payments = WC()->payment_gateways->payment_gateways();
    $usar_formulario_minimizado = $payments['pagopar']->settings['usar_formulario_minimizado'] === 'yes';
    $eliminar_campo_pais = $payments['pagopar']->settings['eliminar_campo_pais'] === 'yes';

    if (is_checkout()) {
        wp_enqueue_script('blocks-checkout-script', plugins_url('/js/checkout-blocks.js', __FILE__), $deps, $version, $in_footer);
        wp_localize_script('blocks-checkout-script', 'checkoutSettings', array(
            'usarFormularioMinimizado' => $usar_formulario_minimizado,
            'eliminarCampoPais' => $eliminar_campo_pais,
        ));
    }

    if (is_cart()) {
        wp_enqueue_script('blocks-cart-script', plugins_url('/js/cart-blocks.js', __FILE__), $deps, $version, $in_footer);
        wp_localize_script('blocks-cart-script', 'cartSettings', array(
            'usarFormularioMinimizado' => $usar_formulario_minimizado,
            'eliminarCampoPais' => $eliminar_campo_pais,
        ));
    }
...
function update_wc_pagopar_billing($array)
{
    ...
    $payments = WC()
        ->payment_gateways
        ->payment_gateways();
    $blocks = CartCheckoutUtils::is_checkout_block_default();
    if (!$blocks) {
...
?>
    ...
    ?>
    <div id='total'><?php echo esc_html(wc_price($_POST['total'])); ?></div>
    <div id='envio'><?php echo esc_html(wc_price($_POST['envio'])); ?></div>
    <?php
    wp_die();
}
    ...
    ?>
            ...
            ?>
                    <tr class="cart_item">
                        <td class="product-name">
                            <a href="<?php echo esc_url(site_url()) . esc_html($product->get_name()); ?>"><?php echo esc_html($product->get_name()); ?></a> &nbsp;
                            <strong class="product-quantity">× <?php echo esc_html($qty); ?></strong>
                        </td>
                        <td class="product-total">
                            ...
                            ?>
                            <span class="woocommerce-Price-amount amount" price="<?php echo esc_attr($value_price); ?>">
                        ...
                        <td class="product-total columnaEnvio">
                            ...
                            ?>
                                            <div class="radio">
                                                <div id="metodo_<?php echo esc_attr($method . '_' . $item->get_id()); ?>">
                                                    <label style="font-size: 11px;">
                                                        <input <?php echo (($costo === 0) and ($method === 'aex')) ? ' onclick="alert(\'Este precio es solo si selecciona AEX en el primer item\');return false;" ' : ''; ?> type="radio" name="envio_seleccionado_<?php echo esc_attr($item->get_id()); ?>" id="envio_seleccionado_<?php echo esc_attr($item->get_id()); ?>"
                                                            identificador="<?php echo esc_attr($method); ?>" class="envio_seleccionado opcion_<?php echo esc_attr($method); ?>"
                                                            price="<?php echo esc_attr($costo); ?>" value="<?php echo esc_attr($method); ?>" <?php echo (($costo === 0) and ($method !== 'aex')) ? "checked" : ''; ?>>
                                                        <span style="text-transform:uppercase;"><?php echo esc_html($method); ?></span>
                                                        <strong class="precio_entrega_<?php echo esc_attr($method); ?>"> - <?php echo esc_html($costo); ?> Gs.</strong><br>
                                                        <em class="entrega_<?php echo esc_attr($method); ?>"><?php echo esc_html($obs); ?></em>
                                                    </label>
                                                </div>
                                            </div>
        ...
        <tfoot>
            ...
            foreach ($taxes as $key => $value):
                echo esc_attr($value->amount);
                echo esc_attr($value->label);
                echo esc_attr($value->formatted_amount);
                $claseTax = 'tax-rate tax-rate-' . str_replace(' ', '-', strtolower($key));
            ?>
                <tr class="tax-rate <?php echo esc_attr($claseTax); ?>">
                    <th>
                        <?php
                        # Array Impuestos
                        $taxes = WC()
                    ...
                    <td><?php echo esc_html($value->formatted_amount); ?></td>
                </tr>
            <?php
            endforeach;
            //var_dump($taxes);
            ...
            <tr class="cart-subtotal" value="0,00">
                ...
                <td>
                    <span class="woocommerce-Price-amount amount">
                        <span class="woocommerce-Price-currencySymbol"><?php echo esc_html($symbol); ?></span>0,00
                    </span>
                </td>
            </tr>
            <tr class="order-total" value="<?php echo esc_html((WC()
 ...
 * @param type $order
 */
function control_courier_crear_pedido_block($order)
{
    // Obtener los datos meta de la orden
    $order_id = $order->get_id();
    $billing_documento = $order->get_meta('_wc_other/pagopar/billing_documento');
    $billing_razon_social = $order->get_meta('_wc_other/pagopar/billing_razon_social');
    $billing_ruc = $order->get_meta('_wc_other/pagopar/billing_ruc');
    $billing_coordenadas = $order->get_meta('_wc_other/pagopar/billing_coordenadas');
    # Tratar documento si esta vacio solo si es pagopar
    if ((substr($order->get_payment_method(), 0, 7) == 'pagopar') && ($order->get_payment_method() != 'pagopar_pix')) {

        $payments = WC()->payment_gateways->payment_gateways();
        $documentoAlternativo = $payments['pagopar']->settings['campo_alternativo_documento'];

        if (($documentoAlternativo != '') && ($documentoAlternativo != 'billing_documento')) {
            $documento_a_tratar = $order->get_meta('_wc_other/pagopar/' . $documentoAlternativo);
        } else {
            $documento_a_tratar = $billing_documento;
        }

        # Obtener método seleccionado, si es mobi las coordenadas deben de ser obligatorias
        $obtenerMetodoEnvioSeleccionadoPagopar = obtenerMetodoEnvioSeleccionadoPagopar();
        if ($obtenerMetodoEnvioSeleccionadoPagopar['metodo_seleccionado'] === "mobi") {
            if (empty($billing_coordenadas)) {
                $array = array(
                    'result' => 'failure',
        ...
        if (empty($documento_a_tratar)) {
            $array = array(
                'result' => 'failure',
                'messages' => 'El documento no puede estar vacío.'
            );
            wp_die(json_encode($array));
        }
        ...
        if (is_numeric($documento_a_tratar)) {
            ...
            $array = array(
                'result' => 'failure',
                'messages' => 'El Documento (CI) no puede ser alfanumérico.'
            );
            wp_die(json_encode($array));
        }
    }
    ...
    $array = array(
        'result' => 'failure',
        'messages' => 'El medio de envío AEX solo está disponible si selecciona cualquiera de los medios de pagos ofrecidos por Pagopar.'
    );

    # Si selecciono AEX y no Pagopar, no se puede finalizar el pedido
    if (strpos($order->get_shipping_method(), "flat_rate_aex") !== false) {
        if (substr($order->get_payment_method(), 0, 7) !== 'pagopar') {
            wp_die(json_encode($array));
        }
    }
    ...
    $array = array(
        'result' => 'failure',
        'messages' => 'El medio de envío MOBI solo está disponible si selecciona cualquiera de los medios de pagos ofrecidos por Pagopar.'
    );

    # Si selecciono MOBI y no Pagopar, no se puede finalizar el pedido
    if (strpos($order->get_shipping_method(), "flat_rate_mobi") !== false) {
        if (substr($order->get_payment_method(), 0, 7) !== 'pagopar') {
            wp_die(json_encode($array));
        }
    }

    if ($order->get_payment_method() == 'pagopar_pix') {
        $payments = WC()->payment_gateways->payment_gateways();
        $documentoAlternativo = $payments['pagopar']->settings['campo_alternativo_documento'];
...
add_filter('woocommerce_store_api_checkout_order_processed', 'control_courier_crear_pedido_block', 1, 2);

function control_courier_crear_pedido_shortcode($null, $order)
{
...
add_filter('woocommerce_checkout_create_order', 'control_courier_crear_pedido_shortcode', 1, 2);
