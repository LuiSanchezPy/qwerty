...
class OrderPagopar {
    ...
    public function addPagoparItem($item) {
        ...
        if ($error['status']) {
            throw new Exception(esc_html($error['msg']));
        }
    ...
    private function validateItemAttributes($name, $qty, $price, $cityId, $productId) {
        ...
        if (empty($name)) {
            $error['status'] = true;
            $error['msg'] = "Hay un error con el nombre de algún producto";
            return $error;
        }
        if (empty($qty) || !is_numeric($qty) || $qty < 0) {
            $error['status'] = true;
            $error['msg'] = "Hay un error en la cantidad del producto '{$name}'";
            return $error;
        }
        if (empty($price) || !is_numeric($price)) {
            $error['status'] = true;
            $error['msg'] = "Hay un error en el precio del producto '{$name}'";
            return $error;
        }
        if (empty($cityId) || !is_numeric($cityId) || $cityId < 0) {
            $error['status'] = true;
            $error['msg'] = "Hay un error en el ID de la ciudad del producto '{$name}'";
            return $error;
        }
        /*if (empty($productId) || !is_numeric($productId) || $productId < 0) {
            $error['status'] = true;
            $error['msg'] = "Hay un error en el ID del producto '{$name}'";
            return $error;
        }*/
        return $error;
    }
    ...
    private function validateBuyerAttributes($name, $email, $cityId) {
        ...
        if (empty($name)) {
            $error['status'] = true;
            $error['msg'] = "Hay un error con el nombre del comprador";
            return $error;
        }
        if (empty($email)) {
            $error['status'] = true;
            $error['msg'] = "Hay un error con el email del comprador";
            return $error;
        }
        /* if(empty($cityId) || !is_numeric($cityId) || $cityId < 0){
    ...
    public function validateOrderAttributes($publicKey, $privateKey, $typeOrder) {
        ...
        if (empty($publicKey)) {
            $error['status'] = true;
            $error['msg'] = "Hay un error con la clave pública";
        }
        if (empty($privateKey)) {
            $error['status'] = true;
            $error['msg'] = "Hay un error con la clave privada";
        }
        //Validamos el tipo de Pedido
        if (empty($typeOrder)) {
            $error['status'] = true;
            $error['msg'] = "Hay un error con el tipo de Pedido";
        }
        return $error;
    }
    ...
    private function makeMaxDateForPayment() {
        ...
        return date("Y-m-d H:i:s", mktime(date("H") + $this->periodOfHoursForPayment + $daysToHours, date("i"), date("s"), date("m"), date("d"), date("Y")));
    }
    ...
    public function makeOrder() {
        ...
        //1 día por default
        $this->periodOfDaysForPayment = ($this->periodOfDaysForPayment >= 0 && is_numeric($this->periodOfDaysForPayment)) ? $this->periodOfDaysForPayment : 1;
        //Solo días y no horas por default
        $this->periodOfHoursForPayment = ($this->periodOfHoursForPayment >= 0 && is_numeric($this->periodOfHoursForPayment)) ? $this->periodOfHoursForPayment : 0;
