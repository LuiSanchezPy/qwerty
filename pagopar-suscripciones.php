...
global $wpdb;
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Consulta directa necesaria para la tabla personalizada wp_pagopar_pagos_automaticos del plugin Pagopar, sin equivalente en APIs de WordPress.
    $ordenes = $wpdb->get_results("SELECT order_id, user_id FROM ".$tabla_pagos." WHERE activo = 1 ORDER BY fecha_creacion ASC");
    
    foreach ($ordenes as $item) {
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Consulta directa necesaria para la tabla personalizada wp_pagopar_pagos_automaticos_detalle del plugin Pagopar, sin equivalente en APIs de WordPress.
            ...
            foreach ($list_tarjetas as $key => $tarjeta) {
              ...
              if($resultado->respuesta == true) {
                ...
                foreach ($orden_detalle as $item_detalle) {
                  ...
                  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Actualización directa necesaria para la tabla personalizada wp_pagopar_pagos_automaticos_detalle del plugin Pagopar, sin equivalente en APIs de WordPress.
