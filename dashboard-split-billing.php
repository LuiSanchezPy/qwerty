...
<div class='update-nag'>La opción de split billing está deshabilitada. <a target="_blank" href="<?php echo esc_url(get_admin_url());?>admin.php?page=wc-settings&tab=checkout&section=pagopar">Favor habilite la opción en el apartado Split Billing</a>.<br />
...
<div style="clear:both;"></div>
<div class='update-nag'>Por cada venta tu comercio ganará <?php echo floatval($payments['pagopar']->settings['porcentaje_comision_comercio_padre']);?>%. Tenés <?php echo esc_html($cantidadComerciosHijos); ?> comercios asociados.
<!--
<p class="submit">
...
?>
<div style="clear:both;"></div>
<div class='update-nag'>Error al listar los medios disponibles ofrecidos por Pagopar, esto muy seguramente se deba a una mala configuración del plugin. <a target="_blank" href="<?php echo esc_url(get_admin_url());?>admin.php?page=wc-settings&tab=checkout&section=pagopar">Favor configure el plugin correctamente</a>.</div>
<div style="clear:both;"></div>
<?php
endif;
?>

<div class="wrap" identificador="zona_<?php echo esc_attr($value2['instance_id']);?>">
	<div>
      ...
      <tr>
		...
		<?php
                ...
                $args = array(
                    ...
                    'meta_query' => array(
                        array(
                            'key' => 'comercio_hijo_vendedor_producto',
                            'compare' => 'EXISTS'
                        )
                    ),
                    'fields' => 'ids'
                ...
                foreach ($product_ids as $post_id) {
                    $meta_value = get_post_meta($post_id, 'comercio_hijo_vendedor_producto', true);
                    if ($meta_value) {
                        if (!isset($meta_counts[$meta_value])) {
                            $meta_counts[$meta_value] = 0;
                        }
                        $meta_counts[$meta_value]++;
                    }
                }
                foreach ($meta_counts as $meta_value => $count) {
                    $productosDB[] = array(
		...
		?>
		<tr>
			<td><?php echo esc_html($value['descripcion']);?></td>
                        <td>
                            <?php 
    ...
    jQuery(function(){
      var $ = jQuery;
        ...
        $( "#split_billing_actualizar" ).on( "click", function() {
            window.location = "<?php echo esc_url(admin_url( 'admin.php?page=wc-settings&tab=checkout&section=pagopar' )); ?>&actualizar-comercios-split-billing"
      });
